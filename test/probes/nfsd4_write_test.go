...
func TestNFSd4Write_Registration(t *testing.T) {
	if !probes.HasProbe("nfsd4_write") {
		t.Fatal("nfsd4_write probe should be registered")
	}

	p, ok := probes.GetProbe("nfsd4_write")
	if !ok {
	...
	if p.Name() != "nfsd4_write" {
		t.Fatalf("expected name nfsd4_write, got %s", p.Name())
	}
}

func TestNFSd4Write_MetadataIntegrity(t *testing.T) {
	p, ok := probes.GetProbe("nfsd4_write")
	if !ok {
		t.Fatal("should get probe from registry")
	}
	...
	if meta.Type != "nfsd4_write" {
		t.Errorf("expected type nfsd4_write, got %s", meta.Type)
	}
	if meta.Title != "NFSD写操作" {
		t.Errorf("expected title 'NFSD写操作', got %s", meta.Title)
	}
	if meta.Layer != "nfsd" {
	...
	// 检查输出字段
	requiredFields := []string{"pid", "lat", "size", "offset", "xid", "comm"}
...
func TestNFSd4Write_Lifecycle(t *testing.T) {
	p, ok := probes.GetProbe("nfsd4_write")
	if !ok {
		t.Fatal("should get probe from registry")
	}
...
func TestNFSd4Write_UpdateWithoutStart(t *testing.T) {
	p, ok := probes.GetProbe("nfsd4_write")
	if !ok {
		t.Fatal("should get probe from registry")
	}
