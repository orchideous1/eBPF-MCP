...
// TestNFSFileReadProbe_Registration 测试探针注册
// 验证 nfs_file_read 探针已正确注册到全局注册表
func TestNFSFileReadProbe_Registration(t *testing.T) {
	// 验证探针已注册
	if !probes.HasProbe("nfs_file_read") {
		t.Fatal("nfs_file_read probe should be registered")
	}

	// 验证可以通过工厂创建实例
	probe, ok := probes.GetProbe("nfs_file_read")
	if !ok {
		t.Fatal("should get probe from registry")
	}

	if probe.Name() != "nfs_file_read" {
		t.Fatalf("expected name 'nfs_file_read', got '%s'", probe.Name())
	}

	// 验证元数据
	meta := probe.GetMetadata()
	if meta.Type != "nfs_file_read" {
		t.Fatalf("expected Type 'nfs_file_read', got '%s'", meta.Type)
	}
	if meta.Title == "" {
...
// TestNFSFileReadProbe_ControllerLifecycle 测试 Controller 生命周期管理
// 验证探针的加载、状态查询、更新、卸载完整流程
func TestNFSFileReadProbe_ControllerLifecycle(t *testing.T) {
	if os.Getuid() != 0 {
	...
	// 1. 测试加载探针
	t.Log("Step 1: 加载探针")
	status, err := controller.Load(ctx, "nfs_file_read")
	if err != nil {
	...
	status, err = controller.Update("nfs_file_read", map[string]any{
...
// TestNFSFileReadProbe_FilterByPID 测试 PID 过滤功能
// 验证 filter_pid 宏变量能正确过滤特定进程的事件
func TestNFSFileReadProbe_FilterByPID(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	defer controller.Unload("nfs_file_read")
	...
	// 触发文件读取操作
	tmpFile := filepath.Join(t.TempDir(), "test_read.txt")
...
// TestNFSFileReadProbe_DataCollection 测试数据收集和持久化
// 验证探针能将事件数据正确写入 DuckDB
func TestNFSFileReadProbe_DataCollection(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	err = db.QueryRow("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'nfs_file_read')").Scan(&tableExists)
	if err != nil {
		t.Logf("无法查询表存在性（可能DuckDB版本不支持）: %v", err)
	} else if !tableExists {
		t.Error("nfs_file_read 表应该在探针加载时创建")
	}
	...
	t.Logf("Database contains %d events (实际数量取决于是否有 NFS 挂载和读取活动)", count)
}

// TestNFSFileReadProbe_MacroVariables 测试宏变量更新
func TestNFSFileReadProbe_MacroVariables(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			status, err := controller.Update("nfs_file_read", tc.config)
			if err != nil {
...
// TestNFSFileReadProbe_ConcurrentAccess 测试并发访问安全性
// 验证多协程同时操作探针时的线程安全性
func TestNFSFileReadProbe_ConcurrentAccess(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	go func() {
		for i := 0; i < 10; i++ {
			controller.Update("nfs_file_read", map[string]any{
	...
	go func() {
		for i := 0; i < 10; i++ {
			controller.Status("nfs_file_read")
			time.Sleep(10 * time.Millisecond)
		}
		done <- true
	}()
	...
	// 协程3：触发文件操作
	go func() {
		tmpFile := filepath.Join(os.TempDir(), "concurrent_test.txt")
...
// TestNFSFileReadProbe_ErrorHandling 测试错误处理
// 验证各种错误场景的正确处理
func TestNFSFileReadProbe_ErrorHandling(t *testing.T) {
	// 测试未注册探针
	t.Log("验证未注册探针...")
	if probes.HasProbe("non_existent_probe") {
		t.Fatal("non_existent_probe should not exist")
	}

	_, ok := probes.GetProbe("non_existent_probe")
	if ok {
		t.Fatal("should not get non-existent probe")
	}
...
func TestNFSFileReadProbe_FullWorkflow(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	t.Log("=== Step 3: 查询探针信息 ===")
	info, err := controller.GetProbeInfo("nfs_file_read")
	if err != nil {
		t.Fatalf("failed to get probe info: %v", err)
	}
	if info.Metadata.Type != "nfs_file_read" {
	...
	t.Log("=== Step 5: 触发文件读取事件 ===")
	tmpFile := filepath.Join(t.TempDir(), "workflow_test.txt")
	...
	rows, err := db.Query("SELECT pid, lat, comm FROM nfs_file_read LIMIT 10")
	if err != nil {
...
// TestNFSFileReadProbe_ListOperations 测试列表操作
// 验证 ListProbes, ListStatus, ListProbeInfos 等批量操作
func TestNFSFileReadProbe_ListOperations(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	// 验证 ListProbes 包含 nfs_file_read 探针
	t.Log("验证 ListProbes...")
	allProbes := probes.ListProbes()
	found := false
	for _, name := range allProbes {
		if name == "nfs_file_read" {
			found = true
			break
		}
	}
	if !found {
		t.Error("nfs_file_read should be in ListProbes")
	}
	...
	if !found {
		t.Error("nfs_file_read should be in ListStatus")
	}
	...
	if !found {
		t.Error("nfs_file_read should be in ListProbeInfos")
	}
...
// TestNFSFileReadProbe_MetadataIntegrity 测试元数据完整性
// 验证探针元数据符合规范要求
func TestNFSFileReadProbe_MetadataIntegrity(t *testing.T) {
	probe, ok := probes.GetProbe("nfs_file_read")
	if !ok {
		t.Fatal("should get probe from registry")
	}
