...
// TestNFSSetattrProbe_Registration 测试探针注册
func TestNFSSetattrProbe_Registration(t *testing.T) {
	if !probes.HasProbe("nfs_setattr") {
		t.Fatal("nfs_setattr probe should be registered")
	}

	probe, ok := probes.GetProbe("nfs_setattr")
	if !ok {
		t.Fatal("should get probe from registry")
	}

	if probe.Name() != "nfs_setattr" {
		t.Fatalf("expected name 'nfs_setattr', got '%s'", probe.Name())
	}

	meta := probe.GetMetadata()
	if meta.Type != "nfs_setattr" {
		t.Fatalf("expected Type 'nfs_setattr', got '%s'", meta.Type)
	}
...
// TestNFSSetattrProbe_ControllerLifecycle 测试 Controller 生命周期管理
func TestNFSSetattrProbe_ControllerLifecycle(t *testing.T) {
	if os.Getuid() != 0 {
	...
	// 1. 测试加载探针
	t.Log("Step 1: 加载探针")
	status, err := controller.Load(ctx, "nfs_setattr")
	if err != nil {
	...
	_, err = controller.Load(ctx, "nfs_setattr")
	...
	// 3. 测试查询状态
	t.Log("Step 3: 查询探针状态")
	status, err = controller.Status("nfs_setattr")
	if err != nil {
	...
	status, err = controller.Update("nfs_setattr", map[string]any{
	...
	// 5. 测试卸载探针
	t.Log("Step 5: 卸载探针")
	status, err = controller.Unload("nfs_setattr")
	if err != nil {
	...
	_, err = controller.Unload("nfs_setattr")
	if !errors.Is(err, probes.ErrProbeNotLoaded) {
		t.Fatalf("expected ErrProbeNotLoaded, got: %v", err)
	}
}

// TestNFSSetattrProbe_FilterByPID 测试 PID 过滤功能
func TestNFSSetattrProbe_FilterByPID(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
	...
	defer controller.Unload("nfs_setattr")

	// 设置 filter_pid 为当前进程 PID
	currentPID := uint32(os.Getpid())
	t.Logf("设置 filter_pid = %d (当前进程)", currentPID)
	_, err = controller.Update("nfs_setattr", map[string]any{
...
// TestNFSSetattrProbe_MacroVariables 测试宏变量更新
func TestNFSSetattrProbe_MacroVariables(t *testing.T) {
	if os.Getuid() != 0 {
		t.Skip("需要 root 权限运行 eBPF 测试")
	}
...
// TestNFSSetattrProbe_MetadataIntegrity 测试元数据完整性
func TestNFSSetattrProbe_MetadataIntegrity(t *testing.T) {
	probe, ok := probes.GetProbe("nfs_setattr")
	if !ok {
		t.Fatal("should get probe from registry")
	}
