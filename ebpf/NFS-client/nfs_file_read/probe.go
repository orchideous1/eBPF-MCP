...
file VARCHAR
	...
	log.Printf("[nfs_file_read] probe started")
	return nil
}
...
func (p *NFSFileReadProbe) consume(ctx context.Context) {
	...
	for {
		...
		select {
		case <-ctx.Done():
			log.Println("[nfs_file_read] context cancelled, exiting consumer")
			return
		default:
		}
		...
		if err := binary.Read(bytes.NewBuffer(record.RawSample), binary.LittleEndian, &event); err != nil {
			log.Printf("[nfs_file_read] parsing event: %v", err)
			continue
		}
		...
		if err != nil {
			log.Printf("[nfs_file_read] appending row: %v", err)
			continue
		}
		...
		if count >= 100 {
			if err := p.appender.Flush(); err != nil {
				log.Printf("[nfs_file_read] flushing appender: %v", err)
			}
			count = 0
		}
	}
...
func (p *NFSFileReadProbe) Stop() error {
	...
	if p.done != nil {
		<-p.done
		// log.Println("[nfs_file_read] consumer exited")
	}
	...
	runCount, _ := probes.SumProgramRunCount(p.objs.NfsFileRead, p.objs.NfsFileReadExit)
	log.Printf("[nfs_file_read] Stop() completed, total triggers: %d, total writes: %d", runCount, atomic.LoadUint64(&p.eventCount))
	return nil
}
