...
//go:generate go tool bpf2go -cflags "-O2" -tags linux bpf nfsd4_write.c -- -I ../../headers
...
func init() {
	probes.Register("nfsd4_write", func() probes.Probe {
		return NewNFSd4WriteProbe()
	})
}

type NFSd4WriteProbe struct {
	probes.BaseProbe
...
func NewNFSd4WriteProbe() *NFSd4WriteProbe {
	// 尝试从 YAML 加载元数据
	meta, exists := probes.GetProbeMetadata("nfsd4_write")
	if !exists {
		// 如果 YAML 中没有，使用默认元数据
		meta = probes.ProbeMetadata{
			...
			Entrypoints: []string{"nfsd4_write"},
	...
	return &NFSd4WriteProbe{
		BaseProbe: probes.NewBaseProbe(meta),
	}
}

func (p *NFSd4WriteProbe) Name() string {
	return "nfsd4_write"
}

func (p *NFSd4WriteProbe) Start(ctx context.Context, db *sql.DB) error {
	if db == nil {
		return logx.ErrDBIsNil
	}

	// 1. Setup DuckDB table and appender
	_, err := db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS nfsd4_write (
pid UBIGINT,
lat UBIGINT,
time_stamp UBIGINT,
size UBIGINT,
...
comm VARCHAR
	...
	p.appender, p.dbConn, err = database.NewDuckDBAppender(ctx, db, "", "nfsd4_write")
	if err != nil {
		p.Stop()
	...
	entryLink, err := link.AttachTracing(link.TracingOptions{
		Program: p.objs.Nfsd4WriteEntry,
	})
	if err != nil {
		p.Stop()
...
func (p *NFSd4WriteProbe) consume(ctx context.Context) {
	defer close(p.done)
	...
	for {
		select {
		case <-ctx.Done():
			log.Println("[nfsd4_write] context cancelled, exiting consumer")
			return
		default:
		}
		...
		if err := binary.Read(bytes.NewBuffer(record.RawSample), binary.LittleEndian, &event); err != nil {
			log.Printf("[nfsd4_write] parsing event: %v", err)
			continue
		}
		...
		if count >= 100 {
			if err := p.appender.Flush(); err != nil {
				log.Printf("[nfsd4_write] flushing appender: %v", err)
			}
			count = 0
		}
	}
...
func (p *NFSd4WriteProbe) Flush() error {
	if p.appender != nil {
...
func (p *NFSd4WriteProbe) Update(config map[string]interface{}) error {
	if config == nil {
		return nil
	}
