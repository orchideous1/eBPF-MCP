...
int BPF_PROG(nfsd4_read_entry, void *rqstp, void *cstate, void *u) {
...
SEC("fexit/nfsd4_read")
int BPF_PROG(nfsd4_read_exit, void *rqstp, void *cstate, void *u, long ret) {
    __u64 pid_tgid = bpf_get_current_pid_tgid();
    ...
    // 获取读操作的偏移量和大小
    // nfsd4_read 结构体: rd_offset (u64), rd_length (u32)
    // u 是 union nfsd4_op_u 指针，read 结构体在其中的偏移与 write 相同
    if (u) {
        bpf_probe_read_kernel(&event->offset, sizeof(event->offset), (void *)((char *)u + 0));
        u32 length = 0;
        bpf_probe_read_kernel(&length, sizeof(length), (void *)((char *)u + 8));
        event->size = (u64)length;
    } else {
        event->offset = 0;
        event->size = 0;
    }
