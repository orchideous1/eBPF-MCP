...
//go:generate go tool bpf2go -cflags "-O2" -tags linux bpf nfsd4_getattr.c -- -I ../../headers
...
func init() {
	probes.Register("nfsd4_getattr", func() probes.Probe {
		return NewNFSd4GetattrProbe()
	})
}

type NFSd4GetattrProbe struct {
	probes.BaseProbe
...
func NewNFSd4GetattrProbe() *NFSd4GetattrProbe {
	// 尝试从 YAML 加载元数据
	meta, exists := probes.GetProbeMetadata("nfsd4_getattr")
	if !exists {
		// 如果 YAML 中没有，使用默认元数据
		meta = probes.ProbeMetadata{
			...
			Layer:       "nfsd",
			Level:       "L2",
			Scene:       "度量NFS服务端(nfsd)处理NFSv4 GETATTR请求的延迟",
			Entrypoints: []string{"nfsd4_getattr"},
	...
	return &NFSd4GetattrProbe{
		BaseProbe: probes.NewBaseProbe(meta),
	}
}

func (p *NFSd4GetattrProbe) Name() string {
	return "nfsd4_getattr"
}

func (p *NFSd4GetattrProbe) Start(ctx context.Context, db *sql.DB) error {
	if db == nil {
		return logx.ErrDBIsNil
	}

	// 1. Setup DuckDB table and appender
	_, err := db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS nfsd4_getattr (
pid UBIGINT,
lat UBIGINT,
time_stamp UBIGINT,
xid UINTEGER,
comm VARCHAR
	...
	p.appender, p.dbConn, err = database.NewDuckDBAppender(ctx, db, "", "nfsd4_getattr")
	if err != nil {
		p.Stop()
	...
	entryLink, err := link.AttachTracing(link.TracingOptions{
		Program: p.objs.Nfsd4GetattrEntry,
	})
	if err != nil {
		p.Stop()
	...
	exitLink, err := link.AttachTracing(link.TracingOptions{
		Program: p.objs.Nfsd4GetattrExit,
	})
	if err != nil {
		p.Stop()
	...
	log.Printf("[nfsd4_getattr] probe started")
	return nil
}

func (p *NFSd4GetattrProbe) consume(ctx context.Context) {
	defer close(p.done)
	...
	for {
		select {
		case <-ctx.Done():
			log.Println("[nfsd4_getattr] context cancelled, exiting consumer")
			return
		default:
		}
		...
		if err := binary.Read(bytes.NewBuffer(record.RawSample), binary.LittleEndian, &event); err != nil {
			log.Printf("[nfsd4_getattr] parsing event: %v", err)
			continue
		}
		...
		if err != nil {
			log.Printf("[nfsd4_getattr] appending row: %v", err)
			continue
		}
		...
		if count >= 100 {
			if err := p.appender.Flush(); err != nil {
				log.Printf("[nfsd4_getattr] flushing appender: %v", err)
			}
			count = 0
		}
	}
...
func (p *NFSd4GetattrProbe) Stop() error {
	if p.cancel != nil {
		p.cancel()
	}
	...
	runCount, _ := probes.SumProgramRunCount(p.objs.Nfsd4GetattrEntry, p.objs.Nfsd4GetattrExit)
	log.Printf("[nfsd4_getattr] Stop() completed, total triggers: %d, total writes: %d", runCount, atomic.LoadUint64(&p.eventCount))
	return nil
}

func (p *NFSd4GetattrProbe) Flush() error {
	if p.appender != nil {
...
func (p *NFSd4GetattrProbe) Update(config map[string]interface{}) error {
	if config == nil {
		return nil
	}
